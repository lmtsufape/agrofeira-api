...
import br.com.ecofeira.domain.repository.FeiraRepository
...
class RelatorioService(
    ...
    fun relatorioPorComerciante(ano: Int, mes: Int?): List<RelatorioComercianteDTO> {
        val feiras = feiraRepository.findAll().filter { feira ->
            feira.dataHora.year == ano &&
                    (mes == null || feira.dataHora.monthValue == mes)
        }

        return feiras.flatMap { feiraComercianteRepository.findByFeiraId(it.id) }
